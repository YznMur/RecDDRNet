...
def to_bayer(img):
    CFA = mosaicing_CFA_Bayer(img)
    o_img = (colour.cctf_encoding(np.clip(CFA, 0, 1)) * 255).astype(np.float32)
    return o_img
...
class BaseDataset(data.Dataset):
    ...
    def gen_sample(self, image, label,
        ...
        cv2.imwrite("/home/trainer/DDRNet.pytorch/input_FIRST.png",image)
        print("input_FIRST",image.shape)
        image = to_bayer(image)
        image.shape = image.shape +(1,)
        width, height = image.shape[:-1]
        # print(image.shape)
        output_array = np.zeros((width, height, 3), dtype=np.float32)
        # print(output_array.shape)
        output_array[:, :, 0]= image[:, :,0]
        output_array[:,:, 1] = image[:, :,0]
        output_array[:, :, 2] = image[:, :,0]
        image=output_array
        cv2.imwrite("/home/trainer/DDRNet.pytorch/input_THREE_BAYER.png",image)
        print("input_THREE_BAYER",image.shape)
        if multi_scale:
        ...
        cv2.imwrite("/home/trainer/DDRNet.pytorch/input_SECOND.png",image)
        print("input_SECOND",image.shape)
