...
# os.environ["CLEARML _OFFLINE_MODE"] = "1"
...
def main():
    args = parse_args()
    # clearml_conf = {'MODE': 'train',
    #                 'DATASET': 'rsm',
    #                 'ARGS': args,
    #                 'CONF': config}

    # task = Task.init(project_name="ddrnet_rsm_v2", task_name="train_segmentation", reuse_last_task_id=False)
    # # task.get_logger().set_reporting_enabled(False)

    # task.connect(clearml_conf)


    task = Task.init(
        project_name="ddrnet_rsm_v2",
        task_name=f"train_segmentation_with_updated_augmentation{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
    ...
    if not hasattr(config.TRAIN, 'RANDOM_ROTATION'):
        config.TRAIN.RANDOM_ROTATION = False
        config.TRAIN.ROTATION_MAX_DEGREE = 0

    if not hasattr(config.TRAIN, 'RANDOM_BLUR'):
        config.TRAIN.RANDOM_BLUR = False

    if not hasattr(config.TRAIN, 'RANDOM_CONTRAST'):
        config.TRAIN.RANDOM_CONTRAST = False

    if not hasattr(config.TRAIN, 'RANDOM_CUTOUT'):
        config.TRAIN.RANDOM_CUTOUT = False
        config.TRAIN.CUTOUT_MAX_SIZE = 0
    ...
    if config.TRAIN.RESUME:
        ...
        checkpoint_files = [
            ...
            if f.startswith("checkpoint") and f.endswith(".pth.tar")
        ]
    ...
    # task.get_logger().report_scalar("loss", "train", iteration=0, value=0)
    task.get_logger().report_scalar(
    "metrics", "validation", iteration=0, value=0
    )
            ...
            if args.local_rank <= 0:
                if mean_IoU > best_mIoU:
                    ...
                    task.upload_artifact(
                        name="best_model",
                        artifact_object=os.path.join(final_output_dir, "best.pth")
                    )
                ...
                task.get_logger().report_scalar(
                    "metrics", "validation", iteration=epoch, value=mean_IoU
                )
                logging.info(msg)
